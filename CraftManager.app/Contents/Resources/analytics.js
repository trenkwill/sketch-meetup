...
if (!panel.get('gaClientId')) {
  panel.set('gaClientId', uuid.v4());
}

// set the cookie storage for google analytics to none, and provide your own client id
var clientId = panel.get('gaClientId');
...
ga('set', 'checkProtocolTask', function(data) {
    data.set('location', 'http://manager.craft.invisionapp.com');
});
ga('set', 'campaignName', panel.getVersion());
ga('send', 'pageview');
